...
Claude Code のメモリ・セッション管理は優秀だが、プロジェクト数が増えるほど人間が把握すべきことも増える。

- Claudeが「これは保存すべき」と判断する基準は曖昧
- ユーザーが気づかないうちに蓄積されると確認が面倒
- 誤った記憶が長期間残ると、むしろ邪魔になる

**何を保存するか** の判断はAIに委ねてよい。しかし **何が保存されているか** を人間が確認し、必要であれば編集できる手段は必要だ。

コンテキストインジェクションをAIの裁量だけに依存しないための、シンプルな可視化レイヤーとしてこのダッシュボードを作った。
