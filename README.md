...
Claude Code のメモリ・セッション管理は優秀であるが、プロジェクト数が増えるほど人間が把握すべきことも増える。

- Claudeが「これは保存すべき」と判断する基準は曖昧である
- ユーザーが気づかないうちに蓄積されると確認が困難になる
- 誤った記憶が長期間残ると、むしろ悪影響を及ぼす

**何を保存するか** の判断は、現状の標準機能としてAIに委ねざるを得ない。しかし **何が保存されているか** を人間が確認し、必要であれば編集できる手段は不可欠である。

コンテキストインジェクションをAIの裁量のみに依存しないための、シンプルな可視化レイヤーとして本ダッシュボードを作るに至った。
