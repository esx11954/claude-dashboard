...
Claude Code のメモリ・セッション管理は優秀であるが、プロジェクト数が増えるほど人間が把握すべきことも増える。Claudeが「これは保存すべき」と判断する基準は曖昧であり、ユーザーが気づかないうちに記憶が蓄積されていく。誤った内容や陳腐化した情報が長期間残り続けた場合、それはコンテキストとして注入され、むしろ悪影響を及ぼす可能性がある。

現状、**何を保存するか** の判断はAIの標準機能に委ねざるを得ない。しかし **何が保存されているか** を人間が把握し、必要であれば編集できる手段は不可欠である。コンテキストインジェクションをAIの裁量のみに依存しないための、シンプルな可視化レイヤーとして本ダッシュボードを作るに至った。
