...
Claude Code のメモリ・セッション管理は優秀であるが、プロジェクト数が増えるほど人間が把握すべきことも増える。Claudeが保存すべきと判断する基準は明確ではなく、気づかないうちに記憶が蓄積される。誤った内容や陳腐化した情報が長期間残り続けた場合それはコンテキストとして注入され、悪影響を及ぼす可能性がある。

現状、**何を保存するか** の判断はAIの標準機能に委ねざるを得ないが、**何が保存されているか** を人間が把握し、場合によって編集できる手段は必要である。

以上を背景としてコンテキストインジェクションをAIの裁量のみに依存しないためのシンプルな可視化レイヤーとして本ダッシュボードを作るに至った。
